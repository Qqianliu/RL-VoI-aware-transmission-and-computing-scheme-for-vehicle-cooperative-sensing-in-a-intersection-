...
def get_snr(distance, P_tx_dBm):
    """
    城市十字路口（UMi）简化模型：
    - 用 P_LOS(d) 确定 LOS/NLOS 的“期望混合”，无随机项，结果连续
    - LOS/NLOS 各自路径损耗指数与固定干扰功率
    - 返回裁剪后的 SINR(dB)，范围 [0, 30]
    """
    # 基本参数
    f_c = 5.9e9
    c = 3e8
...
#     noise_figure_dB = 7  # noise
#     path_loss_exponent = 2.8  #  pathloss factor
#     shadow_fading_sigma = 8  # shadow（dB）
#     # large scale  pathloss
#     PL0 = 20 * np.log10(4 * np.pi / lambda_c)  # 1 m freespace path-loss
#     path_loss_dB = PL0 + 10 * path_loss_exponent * np.log10(distance) \
#                    + np.random.normal(0, shadow_fading_sigma)
...
def main() -> None: 
    ...
    var_action = 1
    agent_ddpg = DDPG(state_dim, action_dim,memory_size=8000,lr_a=0.0001,lr_c= 0.0001)
    lean_start_step = 100
    learning_fre = 1
    ...
    parser.add_argument("--save_dir", type=str, default="./RL-model-scmch0/",
    ...
    for j in range (MAX_EPISODE):
        ...
        for i, data in enumerate(dataflow):
            ...
            print("vehicle image computing delay", v_comp_delay)
            ...
            if i == 0:
                ...
                # scmch = [s_importance,s_aoi/3,decision,sc_state_drift,state_channal]
            ...
            if i== 0:
                ...
                action = np.clip(np.random.normal(action, var_action), -1, 1)
                print(" after explore action", action[0])
                # action = action[0]
            # print("action", action[0])
            var_action  = var_action *  epsilon_decay
            ...
            # print(" feature size", vehicle.data_featue_size)
            raw_trans_delay = vehicle.raw_data_size/uplink_rate
            # print(" raw data size", vehicle.raw_data_size)
            ...
            print(" vehicle ladar and fuse delay S", vehicle_sem_comp_delay  )
            vehicle_sem_comp_energy = vehicle.power_computing *vehicle_sem_comp_delay
            print(" vehicle ladar and fuse energy", vehicle_sem_comp_energy  )
            ...
            print(" rsu feature fusionn and detection energy", rsu_comp_sem_energy)
            rsu_comp_raw_energy = rsu.power_computing * rsu_comp_delay
            rsu_comp_raw_delay = rsu_comp_delay  # it contians all part
            print(" rsu processing all data energy", rsu_comp_raw_energy)
            ...
            if decision ==1:
                ...
                #sem_total_energy  = vehicle_voi_comp_energy * 0.5 + vehicle_sem_comp_energy  + sem_trans_energy + rsu_comp_sem_energy
            ...
            print(f"sem_total_delay : {sem_total_delay:.4f} S")
            print(f"raw_Total_delay : {raw_Total_delay:.4f} S")
            ...
            # enegy_saveing = (raw_total_energy- sem_total_energy)/raw_total_energy

            reward = alpha * accuracy_gain - (1-alpha)* normalized_energy
            # reward = accuracy_gain - normalized_energy  # v2-5 

            # reward =  (alpha *  accuracy_gain) - (1-alpha)* energy_consume
            # accuracy_gain  = res- Infra_accuracy
           #  reward =  alpha * accuracy_gain +  (1-alpha)* enegy_saveing
            ...
            if j == MAX_EPISODE - 1 and i %100 == 0:
                agent_ddpg.save(args.save_dir, episode=j, step=i)
    ...
    with open('Accuray-scmch0.txt', 'w', encoding='utf-8') as f:
        for item in Accuracy:
            f.write(f"{item}\n")
    with open('Decision-scmch0.txt', 'w', encoding='utf-8') as f:
        for item in Decision:
            f.write(f"{item}\n")
    # with open('VoI-t-2.txt', 'w', encoding='utf-8') as f:
    #     for item in VoI:
    #         f.write(f"{item}\n")
    with open('EP-Reward-scmch0.txt', 'w', encoding='utf-8') as f:
        for item in EP_reward:
            f.write(f"{item}\n")
    with open('Energy-scmch0.txt', 'w', encoding='utf-8') as f:
        for item in E2E_energy:
            f.write(f"{item}\n")
    with open('Raw-energy-scmch0.txt', 'w', encoding='utf-8') as f:
        for item in E2E_raw_energy:
            f.write(f"{item}\n")
    with open('Mini-energy-scmch0.txt', 'w', encoding='utf-8') as f:
